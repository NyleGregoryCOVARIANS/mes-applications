...
void FenetreConnexion::on_pushButton_rs232_clicked()
{
    ...
    if (estConnecteSPECS) {
        m_releverMesure->initialisationSPECS(m_communication_SPECS);
    ...
    if (estConnectePICO) {
        m_releverMesure->initialisationPICO(m_communication_PICO);
    ...
    if(estConnecteSPECS && estConnectePICO){
        qDebug() << "Connexion établie, émission du signal connexionEtablie";
        connexionEtablie();
    }
